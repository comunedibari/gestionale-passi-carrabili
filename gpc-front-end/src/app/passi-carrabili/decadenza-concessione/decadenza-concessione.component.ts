...
@Component({
  selector: 'app-decadenza-concessione',
  templateUrl: './decadenza-concessione.component.html',
  styleUrls: ['./decadenza-concessione.component.css']
})
export class DecadenzaConcessioneComponent implements OnInit, OnDestroy {
  ...
  confermaDatiDialog() {
    this.confirmationService.confirm({
      ...
      header: "Decadenza concessione",
      message: "Confermi le motivazioni della richiesta prima di continuare con l'upload dei documenti?",
      accept: () => {
  ...
  inserisciPratica(){
    ...
    pratica.dati_istanza.tipologia_processo = TipologiaPratica.Decadenza;  
  ...
  async inserimentoPraticaGetProtocollo(respInsert){
    ...
    if(this.base64DocInserimento) {
      ...
      this.objTakeEmails = [
        { group_id: Group.DirettoreMunicipio, municipio_id: this.insertResponse.istanza.dati_istanza.indirizzo_segnale_indicatore.municipio_id }
      ];

      if(this.authService.getGroup() != Group.IstruttoreMunicipio)
        this.objTakeEmails.push({ group_id: Group.IstruttoreMunicipio, municipio_id: this.insertResponse.istanza.dati_istanza.indirizzo_segnale_indicatore.municipio_id });

      let callProtocollo: Promise<any> = environment.production ? this.protocolloService.richiestaProtocolloUscitaNewSync(this.objProtocollo, this.objTakeEmails, false, this.base64DocInserimento, convertedUploadedFiles) 
        ...
        this.passiCarrabiliService.inserimentoPratica(element).subscribe( 
          resp => {   
            ...
            if(emailsMunicipio && emailsMunicipio.length) {
              let messaggioMunicipio = this.emailService.emailAvvioPraticaRevoca(resp.istanza);            
