...
@Component({
  selector: 'app-proroga-concessione',
  templateUrl: './proroga-concessione.component.html',
  styleUrls: ['./proroga-concessione.component.css']
})
export class ProrogaConcessioneComponent implements OnInit, OnDestroy {
  ...
  controlloDurataConcessione() {
    ...
    {      
      ...
      if(this.inserisciPraticaForm.get('dati_istanza.anni').value == 0
  ...
  ngOnInit(): void {
    ...
    this.inserisciPraticaForm.get('dati_istanza.motivazione_richiesta').patchValue('');
    this.inserisciPraticaForm.get('dati_istanza.motivazione_richiesta').markAsDirty();
    this.inserisciPraticaForm.get('dati_istanza.motivazione_richiesta').markAsTouched();

    this.inserisciPraticaForm.get('dati_istanza.anni').patchValue(null);
    this.inserisciPraticaForm.get('dati_istanza.anni').markAsDirty();
    this.inserisciPraticaForm.get('dati_istanza.anni').markAsTouched();

    this.inserisciPraticaForm.get('dati_istanza.mesi').patchValue(null);
    this.inserisciPraticaForm.get('dati_istanza.mesi').markAsDirty();
    this.inserisciPraticaForm.get('dati_istanza.mesi').markAsTouched();

    this.inserisciPraticaForm.get('dati_istanza.giorni').patchValue(null);
    this.inserisciPraticaForm.get('dati_istanza.giorni').markAsDirty();
    this.inserisciPraticaForm.get('dati_istanza.giorni').markAsTouched();
  ...
  inserisciPratica(){
    ...
    pratica.dati_istanza.tipologia_processo = TipologiaPratica.Proroga; 
    pratica.proprietario_pratica = this.pratica.proprietario_pratica;
    ...
    let data_scadenza_concessione_temp = this.utilityService.getDataScadenzaConcessione(this.pratica.dati_istanza.data_scadenza_concessione, pratica.dati_istanza.anni, pratica.dati_istanza.mesi, pratica.dati_istanza.giorni);
    pratica.dati_istanza.durata_giorni_concessione = this.utilityService.getGiorniConcessione(this.pratica.dati_istanza.data_scadenza_concessione, data_scadenza_concessione_temp);
    ...
    pratica.dati_istanza.link_pratica_origine = {
      ...
      data_emissione: this.pratica.dati_istanza.link_pratica_origine?.data_emissione || this.pratica.determina?.data_emissione, //TO DO: Forse da invertire
      tag_rfid: this.pratica.tag_rfid || this.pratica.dati_istanza.link_pratica_origine?.tag_rfid
    };  

    if(this.utilityService.checkAvvioProroga(pratica)) {
      this.loading = true;
      ...
      this.messageService.showMessage('warn', 'Proroga concessione', `E' stata superata la durata massima di concessione (1 anno). Il numero massimo di giorni richiedibili con la proroga sono ${365 - pratica.dati_istanza.link_pratica_origine?.durata_giorni_concessione}.`);
    }
  }
  ...
  async inserimentoPraticaGetProtocollo(respInsert){
    ...
    if(!this.objProtocollo) {
      respInsert.istanza.stato_pratica = StatoPraticaPassiCarrabili['Verifica formale'];
      respInsert.istanza.info_passaggio_stato = PassaggiStato.ConcessioneValidaToVerificaFormale;
      ...
      resp => {
        ...
        this.passiCarrabiliService.inserimentoPratica(element).subscribe( 
          resp => {   
            ...
            let messaggioCittadino = this.emailService.emailAvvioPraticaSenzaAssegnazioneCittadino(resp.istanza); 
            ...
            if(emailsMunicipio && emailsMunicipio.length) {
              let messaggioMunicipio = this.emailService.emailAvvioPraticaSenzaAssegnazioneAttori(resp.istanza);            
