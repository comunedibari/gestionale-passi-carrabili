...
@Component({
  selector: 'app-revoca-concessione',
  templateUrl: './revoca-concessione.component.html',
  styleUrls: ['./revoca-concessione.component.css']
})
export class RevocaConcessioneComponent implements OnInit, OnDestroy {
  ...
  confermaDatiDialog() {
    this.confirmationService.confirm({
      ...
      header: "Revoca concessione",
      message: "Confermi la motivazione inserita prima di avviare la revoca della concessione?",
      accept: () => {
  ...
  inserisciPratica(){
    ...
    pratica.dati_istanza.tipologia_processo = TipologiaPratica.Revoca; 
  ...
  async inserimento(respInsert: any) {
    this.loading = true;
    ...
    if(!this.objProtocollo) {
      ...
      this.objProtocollo = this.utilityService.getObjForPdfProtocollo(respInsert.istanza, [], respInsert.istanza.info_passaggio_stato);
      let data: any = await this.utilityService.generaPdfPerProtocolloSync(this.objProtocollo, "templateInserimentoPratica").catch(err => {
        this.actionRiprotocollazione = 'inserimento';
